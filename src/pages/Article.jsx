...
const {id} = useParams()
    ...
    useEffect(() =>{
        let selected = articles.find(p => p.id === parseInt(id)) //  [] | {}
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#0E1217] text-white py-10">
        ...
        <h1 className="text-4xl font-bold text-[#E11D48] text-center mb-5">{selectedArticle.title}</h1>{/* On affiche le titre */}
          <div className="flex flex-col md:flex-row items-center gap-6">
            <img src={selectedArticle.imageUrl} alt={selectedArticle.content} className="rounded-lg shadow-lg max-w-full h-auto md:w-1/2"/>{/* image de l'article */}
            <div>
            
              <h2 className="text-2xl text-[#E11D48] mb-2">Résumé</h2>
              <p className="mb-4">{selectedArticle.summary}</p>
              <div className="flex items-center gap-4 mb-2">
                <img src={selectedArticle.authorImageUrl} alt={selectedArticle.author} className="w-10 h-10 rounded-full shadow-lg"/>
                <span>{selectedArticle.author}</span>
              </div>
              <div className="text-gray-400 mb-2">📅 {new Date(selectedArticle.date).toLocaleDateString()}</div>
              <div className="text-gray-400">⏳ Temps de lecture :{Math.ceil(selectedArticle.length / 300)} mins     </div>
            </div>
          </div>
          <div className="mt-6">
            <h2 className="text-2xl text-[#E11D48] mb-2">{selectedArticle.content}</h2>
