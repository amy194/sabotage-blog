    ...
    <nav className="bg-gray-800 text-white px-4 py-3 border-b-2 border-[#E11D48]"> 
      <div className="container mx-auto flex justify-between items-center flex-wrap">
        ...
        <div className="flex items-center justify-end flex-1">
          <NavLink to="#" className="hover:bg-[#E11D48] px-3 py-2 rounded transition duration-300 ease-in-out">
            Se connecter
          </NavLink>
      ...
      {isOpen && (
        <div className="md:hidden mt-2 space-y-2 px-2 text-center">
          {menuItems.map((item) => (
            <NavLink key={item.title} to={item.link} className="block hover:bg-[#E11D48] px-3 py-2 rounded transition duration-300 ease-in-out">
              {item.title}
            </NavLink>
          ))}
